...
local IMG = "https://cdn.discordapp.com/attachments/1232677825350467624/1311378859635118140/standard.gif?ex=6748a43c&is=674752bc&hm=403fb7ca6166e5a2308e89a8f3038c1ff0add237430d8bddd74820047cd642e9&"
...
ui.sidebar("serenity", icon)



common.add_notify("Serenity ?","Welcome " .. "\a96969696" .. common.get_username())
...
local phase_first = {
    "You just got outplayed by Serenity. Time to uninstall, buddy!",
	"Serenity strikes again! Maybe aim for a tutorial next time.",
    "Relax, Serenity’s got this. You’re just here for the body count.",
    "Guess you’re finding ‘serenity’ in the respawn screen, huh?",
    "Another victim of Serenity’s perfection. Better luck next year!",
    "They said you were good... Serenity disagrees.",
    "Welcome to Serenity’s highlight reel. Smile for the camera!",
    "Oh no, Serenity happened! Looks like your skill didn’t.",
    "Stay calm... Serenity’s just making you irrelevant.",
    "You can’t spell domination without Serenity. Oh wait, yes you can... but you still lost.",
}


local phase_dead = {
    "I’m just looking for some Serenity... but I found death first.",
    "Dead and gone. Serenity’s peace couldn’t save me from that.",
    "Serenity couldn’t handle that shot... guess it’s my time.",
	"I died, but at least Serenity died peacefully… wait, I’m still dead!",
    "Serenity’s peace didn’t save me from that, huh?",
    "Serenity might be peace, but this death... isn’t. See you, world.",
    "Looks like Serenity was too calm for this chaos. Dead again.",
    "Well, Serenity, that was a nice run. See you in the respawn."
}
...
menu = {
    anti_aim = {
        info = {
            ...
            welcome_build = tabs.anti_aim.info2:label("»  Build: \aFFFFFFFFBeta  "..BUILD_VER.."  "..icon.."                         "),
            link_discord = tabs.anti_aim.info2:button("\f<discord> Discord ", function()
                panorama.SteamOverlayAPI.OpenExternalBrowserURL("https://discord.gg/soon")
            end, true),
        ...
        tab_switch = {
            ...
            tab_name = tabs.anti_aim.tab_switch:label("                 \aFFFFFFFF      serenity.lua ® 2024     ")
        },  
        tab_switch_aa = {
        ...
        visuals = {
            ...
            auto_hideshot = tabs.anti_aim.visuals:switch("\v\f<palette>\r  Auto Hide-Shot"),
...
local cfg_data = {
    ints = {
        ...
        menu.anti_aim.visuals.auto_hideshot,
...
exporttcp = tabs.anti_aim.config:button("\f<file-export> Export ", function()
    ...
    common.add_notify("Serenity Beta","successfully exported config to clipboard")
end)
...
importfcp = tabs.anti_aim.config:button("\f<file-import> Import ", function()
    ...
    cfg(config)
    common.add_notify("Serenity Beta","successfully imported config from clipboard")
end)
...
defaultpreset = tabs.anti_aim.config:button("\f<loader> Default ", function()
    ...
    cfg(default)
    common.add_notify("Serenity Beta","successfully loaded default config")
end)
...
loadcfg = tabs.anti_aim.config:button("\f<file-arrow-up> Load ", function()
    ...
    cfg(config_tl)
    common.add_notify("Serenity Beta","successfully loaded config  " .. "\aFFFFF0FF" .. db.configs[menu.anti_aim.config.config_id:get()][2])
end)
...
overwritecfg = tabs.anti_aim.config:button("\f<user-pen> Save ", function()
    ...
    db.configs = configs
    common.add_notify("Serenity Beta","successfully overwrited config  " .. "\aFFFFF0FF" .. configs[menu.anti_aim.config.config_id:get()][2])
end)
...
deletecfg = tabs.anti_aim.config:button("\f<trash-can> Remove ", function()
    ...
    db.configs = configs
    common.add_notify("Serenity Beta","successfully removed config  " .. "\aFFFFF0FF" .. config_removed[2])
end)
...
savecfg = tabs.anti_aim.config:button("\f<circle-plus> Create ", function()
    ...
    if savecfg_1 == 2 then
        ...
        menu.anti_aim.config.config_name:visibility(false)
        savecfg_1 = 0
        common.add_notify("Serenity Beta","successfully saved config  " .. "\aFFFFF0FF" .. menu.anti_aim.config.config_name:get())
...
local function auto_baim()
    if not menu.anti_aim.visuals.auto_baim:get() then
        return
    end
    ...
    local hp = threat.m_iHealth
    if hp <= 0 then
        return
    end

    -- Check if the enemy has a safe head
    local has_safe_head = threat:has_safe_head()  -- This is a hypothetical function; implement your own logic

    -- Set body aim overrides based on health and safe head status
    if hp <= 90 or (has_safe_head and hp > 90) then
...
logger.miss = function(e)
    ...
    if menu.anti_aim.visuals.aimlogs.aimlogs_type:get("Console") and menu.anti_aim.visuals.aimlogs:get() then
        print_dev((
            "serenity ● missed \ad6b9a55 " .. e.target:get_name() .. " \ad2d6d22 due to \ad6b9a55 " .. state .. " \ad2d6d22 in \ad6b9a55 " .. logger.hitgroup[ e.wanted_hitgroup ] .. " \ad2d6d22 bt: \ad6b9a55 " .. e.backtrack .. " \ad2d6d22"
        ));
...
logger.hit = function(e)
    if menu.anti_aim.visuals.aimlogs.aimlogs_type:get("Console") and menu.anti_aim.visuals.aimlogs:get() then
        print_dev((
           "serenity ● hit \acad6a55 " .. e.target:get_name() .. " \ad2d6d22 for \acad6a55 " ..  e.damage .. " \ad2d6d22 in: \acad6a55 " ..  logger.hitgroup[ e.hitgroup ] .. " \ad2d6d22 (".."exp: \acad6a55 " .. e.wanted_damage .. " \ad2d6d22 bt: \acad6a55 " .. e.backtrack .."\ad2d6d22 )"
        ));
...
set_visibility = function()
    ...
    menu.anti_aim.visuals.auto_hideshot:visibility(menu.anti_aim.tab_switch.tab_switcher:get() == 4)
...
local function gs_ind()
    ...
    if menu.anti_aim.private.gs_ind:get() then
            ...
            if find.ping:get() > 0 then
                render_indicators("Ping Abuser", color(150, 189, 18, 255), ind_dst)
                ind_dst = ind_dst + ind_spr
            end
        
            if refs.doubletap:get() then
                render_indicators("100% Jesus", rage.exploit:get() == 1 and color(215, 215, 215, 255) or color(255, 0, 0, 255), ind_dst)
                ind_dst = ind_dst + ind_spr
            elseif refs.hideshots:get() and not refs.doubletap:get()  then
                render_indicators("Hide Shots", color(215, 215, 215, 255), ind_dst)
                ind_dst = ind_dst + ind_spr
            end
            
            if refs.freestanding:get() or menu.anti_aim.condition.conditions.fs_aa:get() then
                render_indicators("Freestand", color(215, 215, 215, 255), ind_dst)
                ind_dst = ind_dst + ind_spr
            end
        
            if refs.isBody:get() == "Force" then
                render_indicators("Baim Abuser", color(215, 215, 215, 255), ind_dst)
                ind_dst = ind_dst + ind_spr
            end
        
            if get_bind("Min. Damage")[1] then
                render_indicators("Min Damage Warrior", color(215, 215, 215, 255), ind_dst)
                ind_dst = ind_dst + ind_spr
            end
        
            if get_bind("Hit Chance")[1] then
                render_indicators("H1TCHANCE", color(215, 215, 215, 255), ind_dst)
                ind_dst = ind_dst + ind_spr
            end
                
            if refs.fakeduck:get() then
                render_indicators("Real Crouch", color(215, 215, 215, 255), ind_dst)
                ind_dst = ind_dst + ind_spr
            end
...
local function watermark()
    if menu.anti_aim.private.watermark.watermark_type:get() == "Modern" then
        ...
        render.text(1,vector(27,render.screen_size().y/2-130),color(180,180,180),"","S e r e n i t y")
    end
end
local function watermark2()
    ...
    render_text(2, vector(render_screen_size().x/40, render_screen_size().y/2.04), color(), nil, "serenity.lua")
    render_text(2, vector(render_screen_size().x/40, render_screen_size().y/2.01), color(), nil, "[Beta]")
end
end
...
events.render:set(function()
    local gradient_animation = libs.gradient.text_animate("Serenity Beta", -0.9, {
        color(98,118,156),
