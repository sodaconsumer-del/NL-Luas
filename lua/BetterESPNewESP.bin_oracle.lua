...
local v1 = require("neverlose/bomb_lib")("Zml4X3RoZV9kdW1wZXI=");
...
local v217 = esp.enemy:new_text("Bomb Information", "Bomb", function(v211)
    ...
    if bomb_options:get("Planting") and v1.state == 1 and v1.planter == v211 then
        local _, v213 = v1:get_plant_duration();
        return "Planting: " .. tostring(math.floor(v213 + 0.5)) .. "%";
    elseif bomb_options:get("Defuse") and v1.state == 3 and v211.m_bIsDefusing then
        if v1:can_defuse(v211) then
            local _, v215 = v1:get_defuse_duration();
    ...
    elseif bomb_options:get("Carry") and (v211 == entity.get(v211:get_resource().m_iPlayerC4) or v46(v211, 49)) then
        ...
        if bomb_options:get("Damage") and v1.state == 2 then
            if v211:is_dormant() then
                return;
            else
                local v216 = v1:calculate_damage(v211);
                if v216 > 0 then
                    if v211.m_iHealth <= v216 then
                        return v1.site .. ": Fatal";
...
local function v329()
    -- upvalues: v6 (ref), v0 (ref), v328 (ref)
    files.create_folder(v6 .. "/materials");
    if not v0.exists(v6 .. "/materials/thunder.png") then
        v0.download("https://gitlab.com/luascript/csgo/-/raw/main/misc/thunder.png?inline=false", v6 .. "/materials/thunder.png");
    end;
...
local function v371()
    ...
    if ui.get_alpha() > 0 then
        ...
        v13.features.last_dormant_position.switch:name(v8.primary .. v12["clock-rotate-left"] .. v8.default .. "   Last Dormant Position");
